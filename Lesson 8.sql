...
-- INNER JOIN
-- Это самый простой и часто используемый вариант команды — внутреннее объединение. 
-- Если режим работы операции не указан вручную, то SQL автоматически воспримет любой JOIN как внутренний.
-- Оператору передаются две таблицы, и он возвращает их внутреннее пересечение по какому-либо критерию. 
-- Результатом будут записи, которые соответствуют обеим таблицам, — их перед отправкой объединят.


-- Вывести имя, фамилию сотрудников, имя и адресс(street_address, city) департамента, в котором они работают 
select t1.first_name, t1.last_name, t2.department_name, t3.street_address
from employees t1
join departments t2
...
-- from table1 t1
-- [inner/left/right] join table1 t2
-- on t1.column1 = t2.column2; 

-- Это «самосоединение», объединение внутри одной таблицы. 
-- Оно используется тогда, когда у разных полей одной таблицы могут быть одинаковые значения. 
-- Например, один и тот же участник музыкальной группы может быть и вокалистом, и, например, клавишником. 
-- Если из базы музыкальных групп понадобится извлечь те, где вокалист и клавишник — одно лицо, 
-- потребуется Self Join.

-- Эта вариация может быть и внутренней, и внешней. Ее отличие в том, 
-- что таблица при таком режиме присоединяется сама к себе. Без практики это может быть непривычно, 
-- но в процессе использования логику работы легко понять.

-- Чтобы Self Join работал правильно, могут потребоваться псевдонимы таблиц: 
-- они помогают называть одну и ту же таблицу разными именами. 
-- В результате оператор «воспринимает» переданные сущности как разные.

-- Смотрим под капот
-- В работе с self join использование псевдонимов аналогично использованию спасательной шлюпки на корабле, 
-- чтоб избежать столкновения с "айсбергами" неопределённости идентификаторов колонок.
...
select t1.first_name as emp_name, t1.last_name as emp_last_name,
t2.first_name as mng_name, t2.last_name as mng_last_name
from employees t1
join employees t2
...
from employees all_emp -- все сотрудники, кроме 'Luis' 'Popp'
join employees L_P -- только 'Luis' 'Popp'
on all_emp.employee_id != L_P.employee_id -- чтобы их ИД не совпадали
where L_P.first_name = 'Luis' and L_P.last_name = 'Popp' and all_emp.salary > L_P.salary; 
-- из таблицы L_P по условию находим только 'Luis' 'Popp' и сравниваем его зарплату с остальними

-- Решение с помощью подзапросов

-- 1. Найти зарплату 'Luis' 'Popp'
select salary
from employees
where first_name = 'Luis' and last_name = 'Popp';
...
-- select column1, column2,..
-- from table1
-- where column = (select column from table2 where ...); -- поздапрос с одним полем

-- 2. Вывести тех, которые получают больше чем 'Luis' 'Popp'
select first_name, last_name, salary
from employees
...
select t1.first_name, t1.last_name, t1.salary
from employees t1 -- all employees
join employees t2 -- Adam Fripp
...
-- Вывести тех сотрудников, у которых нет менеджеров.
select t1.first_name as emp_first_name, t1.last_name as emp_last_name
from employees t1 -- employees
left join employees t2 -- managers
on t1.manager_id = t2.employee_id
where t2.employee_id is null;
...
and first_name != 'Laura' and last_name != 'Bissot';

select t1.first_name, t1.last_name
from employees t1 -- all emp
join employees t2 -- Laura
on t1.employee_id != t2.employee_id
where t2.first_name = 'Laura' and t2.last_name = 'Bissot' and t1.department_id = t2.department_id;
