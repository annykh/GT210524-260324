...
select year('2025-09-25');

select extract(year from '2025-09-25');
...
-- Значение format:
    ...
    -- %r: время в 12-ти часовом формате (hh:mm:ss AM/PM)
    -- %T: время в 12-ти часовом формате (hh:mm:ss)
    
set lc_time_names = 'ru_RU';

select date_format('2025-09-25', '%M');
...
select hire_date, date_format(hire_date, '%d %M %y') as new_format
from employees;

create table month_names(
	month_id int primary key auto_increment,
    month_name_en varchar(60),
    month_name_ru varchar(60)
);

insert into month_names(month_name_en, month_name_ru)
values ('January', 'Январь'),
		('February', 'Февраль'),
        ('March', 'Март'),
        ('April', 'Апрель'),
        ('May', 'Май'),
...
select t1.hire_date, t2.month_name_ru
from employees t1
join month_names t2
on date_format(t1.hire_date, '%M') = t2.month_name_en;

select t1.hire_date, t2.month_name_ru
from employees t1
join month_names t2
on date_format(t1.hire_date, '%c') = t2.month_id;

select t1.hire_date, t2.month_name_ru
from employees t1
join month_names t2
on monthname(hire_date) = t2.month_name_en;

select t1.hire_date, t2.month_name_ru
from employees t1
join month_names t2
on month(hire_date) = t2.month_id;

-- Из employees вывести имена и фамилии сотрудников которых приняли на работу в марте.
...
select first_name, last_name, date_format(hire_date, '%m')
from employees
where date_format(hire_date, '%m') = '03';

-- Из employees вывести имена и фамилии сотрудников, а также даты, когда их приняли на работу в формте 
-- 'День_месяца Название_месяца(на англ.) Год(две цифры)'
select first_name, last_name, date_format(hire_date, '%e %M %y') as hire_date
from employees;

-- Из employees вывести имена и фамилии сотрудников, а также даты, когда их приняли на работу в формте 
-- 'День_месяца Название_месяца(на русском) Год(две цифры)'
select t1.hire_date, concat_ws(' ', date_format(t1.hire_date, '%e'), t2.month_name_ru, date_format(t1.hire_date, '%y')) as hire_date
from employees t1
join month_names t2
on date_format(t1.hire_date, '%M') = t2.month_name_en;
